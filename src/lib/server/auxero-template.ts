...
const primaryPageToFile = {
	home: 'home-05.html',
	inventory: 'listing-grid3-columns.html',
...
const canonicalTemplateRoutes: Record<string, string> = {
	...
	'listing-grid4-columns.html': '/inventory?view=4',
...
function applyBohemcarsBranding(html: string) {
	return html
		...
		.replaceAll('My Listing', 'My Listings')
...
`
	...
	body.${pageClass} .bohemcars-inventory-searchbar__chips {
		display: flex;
		flex-wrap: wrap;
		gap: 6px;
		margin-top: 8px;
...
(() => {
	...
	const favoriteCardHtml = (vehicle, index) => {
		...
		const favoriteIconHtml = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M16.69 3.75C14.75 3.75 13.06 4.58 12 5.99C10.94 4.58 9.25 3.75 7.31 3.75C5.77 3.75 4.29 4.36 3.2 5.45C2.11 6.54 1.5 8.02 1.5 9.56C1.5 16.13 11.23 21.44 11.64 21.66C11.75 21.72 11.88 21.75 12 21.75C12.12 21.75 12.25 21.72 12.36 21.66C12.77 21.44 22.5 16.13 22.5 9.56C22.5 8.02 21.89 6.54 20.8 5.45C19.71 4.36 18.23 3.75 16.69 3.75Z" fill="#d9f275" stroke="#d9f275" stroke-width="1.5"/></svg>';
		return '<div class="card-box card-box-style-1" data-bohemcars-slug="' + escapeText(vehicle.slug) + '">' +
			'<div class="top"><p class="' + tagClass + ' text-white highlight">' + escapeText(vehicle.condition || 'Available') + '</p><p class="heart bohemcars-favorite is-active" role="button" tabindex="0" aria-label="Remove ' + escapeText(vehicle.title) + '">' + favoriteIconHtml + '</p></div>' +
			...
			'<div class="content border-light border-top-none">' +
				'<div class="bottom"><p class="category uppercase text-white"><a href="/inventory/' + encodeURIComponent(vehicle.slug) + '" class="text-white uppercase text-xs">' + escapeText(vehicle.brand) + '</a></p></div>' +
	...
	const inventorySearchUrl = (form) => {
		...
		for (const [key, value] of [...params.entries()]) {
			...
			if (!normalized || normalized === 'all' || (key === 'view' && normalized === '3')) {
				params.delete(key);
			}
		}
...
export function renderAuxeroTemplate(templateFile: string, options: AuxeroRenderOptions = {}) {
	...
	return injectLocalBehavior(withData, templateFile, options);
}
