...
const applyAddListingData = (
	...
	let next = applyAccountShell(html, templateFile, options)
		...
		.replace(
			...
			`<a href="#" class="btn btn-primary px-24 btn-large font-weight-600 bohemcars-local-form-action" data-bohemcars-form-target=".bohemcars-add-listing-form" data-bohemcars-form-status="Listing published locally for Bohemcars" data-bohemcars-submit-form="true" data-bohemcars-listing-status="${primaryListingStatus}">`
		)
		.replace(
			'<input type="file" id="carPreviewInput" accept="image/jpeg,image/png,image/jpg" class="car-preview-upload__input">',
			'<input type="file" id="carPreviewInput" name="previewImage" accept="image/jpeg,image/png,image/jpg,image/webp" class="car-preview-upload__input">'
		)
		.replace(
			'<input type="file" id="carGalleryInput" accept="image/jpeg,image/png,image/jpg" multiple class="car-gallery-upload__input">',
			'<input type="file" id="carGalleryInput" name="galleryImages" accept="image/jpeg,image/png,image/jpg,image/webp" multiple class="car-gallery-upload__input">'
		)
		.replace(
			'<input type="file" id="attachmentsInput" accept=".pdf,.doc,.docx" multiple class="car-gallery-upload__input" style="display: none;">',
			'<input type="file" id="attachmentsInput" name="documents" accept=".pdf,.doc,.docx" multiple class="car-gallery-upload__input" style="display: none;">'
		)
