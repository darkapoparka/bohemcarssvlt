<script lang="ts">
	...
	let { tabs }: { tabs: AuxeroVehicleDetailFeatureTab[] } = $props();
</script>
...
<div class="flat-tabs mb-40">
	...
	<div class="content-tab">
		{#each tabs as tab, tabIndex (tab.label)}
			<div class={tabIndex === 0 ? 'content-inner active' : 'content-inner '}>
				<ul class="xl-grid-cols-2 md-grid-cols-1 grid grid-cols-3 gap-8 gap-x-30">
					{#each tab.items as feature, featureIndex (`${tab.label}-${featureIndex}`)}
