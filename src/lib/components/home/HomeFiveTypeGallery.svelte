<script lang="ts">
	...
	import type { HomeFiveTypeCard } from '$lib/auxero/home-five';

	let { cards }: { cards: HomeFiveTypeCard[] } = $props();
</script>

<section class="bg-primary relative py-100">
	<div class="relative container">
		<div class="title-section wow fadeInDown mb-42" data-wow-delay="0.1s">
			<h2 class="text-center text-white">Browse By Type</h2>
			<a
				href={resolve('/inventory?view=4')}
				class="btn btn-blur hover-fill-primary font-weight-600 btn-large"
			>
				Check All Car Type
				<svg
					width="20"
				...
				>
					<g clip-path="url(#clip0_16987_736)">
						<path
	...
	<div class="container">
		<div class="container-grid-gallery gallery-scroll">
			<div class="slide-gallery-list scroll">
				{#each cards as typeCard, index (typeCard.image)}
					<div
						class={`brand-item-style-2 slide-gallery ${index === 1 ? 'active ' : ''}cursor-pointer`}
					>
						<img src={typeCard.image} alt={typeCard.label} />
						<a
							class="brand-item-button"
							href={resolve(`/inventory?bodyType=${encodeURIComponent(typeCard.bodyType)}`)}
						>
							{typeCard.label}
						</a>
					</div>
