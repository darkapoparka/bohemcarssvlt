<script lang="ts">
	...
	let { pairs }: { pairs: HomeFiveComparePair[] } = $props();
</script>
	...
	<div class="container">
		...
		<div class="swiper-container swiper-card-2 wow fadeIn" data-wow-delay="0.1s">
						...
						<div class="card-box-style-4 bohemcars-calm-hover">
							...
							<div class="content">
								<div class="mb-12 flex justify-between gap-12">
									<div>
										<p class="text-muted text-underline mb-4 text-xs uppercase">
											{pair.left.brand}
										</p>
										<p class="h7">{pair.left.title}</p>
										<p class="h6">{pair.left.priceLabel}</p>
									</div>
									<div class="text-right">
										<p class="text-muted text-underline mb-4 text-xs uppercase">
											{pair.right.brand}
										</p>
										<p class="h7">{pair.right.title}</p>
										<p class="h6">{pair.right.priceLabel}</p>
									</div>
								</div>
								<a
									href={resolve(
									...
									class="btn btn-small btn-line-1 text-sm"
								>
									<svg
										width="20"
										height="20"
			...
			<div class="md-block hidden">
				<div
					class="swiper-pagination pagination-dark pagination-style pagination-swiper-card-2 mt-35"
				></div>
			</div>
		</div>
	</div>
