			...
			<div>
				...
				<form class="relative max-w-[340px]" action="#">
					<input
						...
						class="h-14 w-full rounded-full border border-[#9fa1a4] bg-white/10 pr-16 pl-5 text-sm text-white outline-none transition-[border-color,box-shadow] duration-200 placeholder:text-[#9fa1a4] hover:border-white/60 focus:border-bc-accent focus:shadow-[0_0_0_3px_rgba(152,188,42,0.35)]"
						required
					/>
					<button
						type="submit"
							...
							{#each group.links as link (link.href)}
								<li>
									<a
										...
										class="text-sm leading-[22px] text-[#9fa1a4] transition-colors hover:text-white"
									>
										{link.label}
									</a>
								</li>
			...
			<ul class="flex flex-wrap items-center justify-center gap-x-6 gap-y-1">
				{#each footer.legalLinks as link (link.label)}
					<li>
						<a
							href={resolve(link.href as '/')}
							class="text-sm text-[#9fa1a4] transition-colors hover:text-white"
						>
							{link.label}
						</a>
					</li>
